...
use App\Exports\Master\BankExport;
use App\Models\Master\Bank;
use App\Models\User\UserFilter;
use App\Tables\Master\BankTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Banks', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Banks', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'bank-export'],
    ],
]);
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    ...
    return Excel::download(new BankExport($rows), 'banks.xlsx');
};

$deleteBank = function (int $recordId): bool {
    $bank = Bank::query()->findOrFail($recordId);
    $bank->delete();
    ...
    $this->dispatch(
        'notify',
        title: 'Bank deleted',
        message: 'The bank has been successfully deleted.'
    );
...
on([
    ...
    'bank-export' => $export,
    'bank-delete-confirmed' => $deleteBank,
]);
...
$banks = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
    return Bank::query()
        ->search($this->search, $this->searchableFields)
...
?>
<x-layouts.app :title="__('Master : Bank')">
@volt('apps.master.banks.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-bank" wire:click="$dispatch('bank-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Bank</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Bank</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Master\BankTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.master.bank"
        ...
        <livewire:apps.form.master.bank/>
    </flux:main>
@endvolt
</x-layouts.app>
