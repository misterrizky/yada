...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Cities', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Cities', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'city-export'],
    ],
]);
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    ...
    return Excel::download(new CityExport($rows), 'cities.xlsx');
};

$deleteCity = function (int $recordId): bool {
    $city = City::query()->findOrFail($recordId);
    $city->delete();
...
on([
    ...
    'city-export' => $export,
    'city-delete-confirmed' => $deleteCity,
]);
...
$cities = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
    $query = $this->state
        ? $this->state->cities()->getQuery()
        : City::query();
...
$chartData = computed(fn (): \Illuminate\Support\Collection => City::query()
    ->when($this->state, fn ($q) => $q->where('state_id', $this->state->id))
...
?>
<x-layouts.app :title="__('Regional : City')">
@volt
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-city" wire:click="$dispatch('city-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>{{ $this->state->country?->name ?? 'Country' }}</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>State</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>{{ $this->state->name }}</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>City</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">City</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Regional\CityTable::columns()"
                :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.regional.city"
        ...
        <livewire:apps.form.regional.city/>
    </flux:main>
@endvolt
</x-layouts.app>
