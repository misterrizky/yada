...
use App\Exports\Master\TaxRateExport;
use App\Models\Finance\TaxRate;
use App\Models\User\UserFilter;
use App\Tables\Master\TaxRateTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Tax Rates', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Tax Rates', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'tax-rate-export'],
    ],
]);
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    ...
    return Excel::download(new TaxRateExport($rows), 'tax-rates.xlsx');
};

$deleteTaxRate = function (int $recordId): bool {
    $record = TaxRate::query()->findOrFail($recordId);
    $record->delete();
    ...
    $this->dispatch(
        ...
        message: 'The tax rate has been successfully deleted.'
    );
...
on([
    'tax-rate-saved' => function () {
    },
    'tax-rate-export' => $export,
    'tax-rate-delete-confirmed' => $deleteTaxRate,
]);
...
$taxRates = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
...
?>
<x-layouts.app :title="__('Master : Tax Rates')">
@volt('apps.master.tax-rates.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-tax-rate" wire:click="$dispatch('tax-rate-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Tax Rates</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Tax Rates</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Master\TaxRateTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.master.tax-rate"
        ...
        <livewire:apps.form.master.tax-rate/>
    </flux:main>
@endvolt
</x-layouts.app>
