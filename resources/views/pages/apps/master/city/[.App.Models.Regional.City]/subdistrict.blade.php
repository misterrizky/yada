...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Subdistricts', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Subdistricts', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'subdistrict-export'],
    ],
]);
...
$storeUserFilter = function (\Livewire\Component $component) use ($currentViewSettings): void {
    ...
    UserFilter::updateOrCreate(
        ['user_id' => $userId, 'key' => 'subdistricts'],
        ['value' => [
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    $query = $this->city
        ? $this->city->subdistricts()->getQuery()
        : Subdistrict::query();
    ...
    return Excel::download(new SubdistrictExport($rows), 'subdistricts.xlsx');
};

$deleteSubdistrict = function (int $recordId): bool {
    $subdistrict = Subdistrict::query()->findOrFail($recordId);
    $subdistrict->delete();

    $this->dispatch('modal-close', name: 'delete');
    $this->dispatch(
        'notify',
        title: 'Subdistrict deleted',
        message: 'The subdistrict has been successfully deleted.'
    );
...
$subdistricts = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
    $query = $this->city
        ? $this->city->subdistricts()->getQuery()
        : Subdistrict::query();
...
$chartData = computed(fn (): \Illuminate\Support\Collection => Subdistrict::query()
    ->when($this->city, fn ($q) => $q->where('city_id', $this->city->id))
...
?>
<x-layouts.app :title="__('Regional : Subdistrict')">
@volt
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-subdistrict" wire:click="$dispatch('subdistrict-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>{{ $this->city->state?->country?->name ?? 'Country' }}</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>State</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>{{ $this->city->state?->name ?? 'State' }}</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>City</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>{{ $this->city->name }}</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>Subdistrict</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Subdistrict</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Regional\SubdistrictTable::columns()"
                :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                :columns="$columns"
                :rows="$this->subdistricts"
                :sort-field="$this->sortField"
                :sort-direction="$this->sortDirection"
                sort-action="sortBy"
                actions-view="pages.apps.actions.regional.subdistrict"
        ...
        <livewire:apps.form.regional.subdistrict/>
    </flux:main>
@endvolt
</x-layouts.app>
