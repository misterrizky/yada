...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import States', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export States', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'state-export'],
    ],
]);
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    ...
    return Excel::download(new StateExport($rows), 'states.xlsx');
};

$deleteState = function (int $recordId): bool {
    $state = State::query()->findOrFail($recordId);
    $state->delete();
...
on([
    ...
    'state-export' => $export,
    'state-delete-confirmed' => $deleteState,
]);
...
$states = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
    $query = $this->country
        ? $this->country->states()->getQuery()
        : State::query();
...
$chartData = computed(fn (): \Illuminate\Support\Collection => State::query()
    ->when($this->country, fn ($q) => $q->where('country_id', $this->country->id))
...
?>
<x-layouts.app :title="__('Regional : State')">
@volt
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-state" wire:click="$dispatch('state-create')">
        ...
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">State</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Regional\StateTable::columns()"
                :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.regional.state"
