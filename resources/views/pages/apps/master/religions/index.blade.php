...
use App\Exports\Master\ReligionExport;
use App\Models\Master\Religion;
use App\Models\User\UserFilter;
use App\Tables\Master\ReligionTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Religions', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Religions', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'religion-export'],
    ],
]);
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    ...
    return Excel::download(new ReligionExport($rows), 'religions.xlsx');
};

$deleteReligion = function (int $recordId): bool {
    $record = Religion::query()->findOrFail($recordId);
    $record->delete();
    ...
    $this->dispatch(
        ...
        message: 'The religion has been successfully deleted.'
    );
...
on([
    'religion-saved' => function () {
    },
    'religion-export' => $export,
    'religion-delete-confirmed' => $deleteReligion,
]);
...
$religions = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
...
?>
<x-layouts.app :title="__('Master : Religions')">
@volt('apps.master.religions.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-religion" wire:click="$dispatch('religion-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Religions</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Religions</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Master\ReligionTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.master.religion"
        ...
        <livewire:apps.form.master.religion/>
    </flux:main>
@endvolt
</x-layouts.app>
