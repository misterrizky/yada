...
use App\Exports\Master\IndustryExport;
use App\Models\Master\Industry;
use App\Models\User\UserFilter;
use App\Tables\Master\IndustryTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Industries', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Industries', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'industry-export'],
    ],
]);
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    ...
    return Excel::download(new IndustryExport($rows), 'industries.xlsx');
};

$deleteIndustry = function (int $recordId): bool {
    $record = Industry::query()->findOrFail($recordId);
    $record->delete();
    ...
    $this->dispatch(
        ...
        message: 'The industry has been successfully deleted.'
    );
...
on([
    'industry-saved' => function () {
    },
    'industry-export' => $export,
    'industry-delete-confirmed' => $deleteIndustry,
]);
...
$industries = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
...
?>
<x-layouts.app :title="__('Master : Industries')">
@volt('apps.master.industries.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-industry" wire:click="$dispatch('industry-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Industries</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Industries</flux:heading>
                <flux:input
        ...
        <livewire:apps.form.master.industry/>
    </flux:main>
@endvolt
</x-layouts.app>
