...
use App\Exports\Master\StageExport;
use App\Models\Master\Stage;
use App\Models\User\UserFilter;
use App\Tables\Master\StageTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Stages', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Stages', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'stage-export'],
    ],
]);
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    ...
    return Excel::download(new StageExport($rows), 'stages.xlsx');
};

$deleteStage = function (int $recordId): bool {
    $stage = Stage::query()->findOrFail($recordId);
    $stage->delete();
    ...
    $this->dispatch(
        'notify',
        title: 'Stage deleted',
        message: 'The stage has been successfully deleted.'
    );
...
on([
    'stage-saved' => function () {
    },
    'stage-export' => $export,
    'stage-delete-confirmed' => $deleteStage,
]);
...
$stages = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
    return Stage::query()
        ->with('pipeline')
        ->search($this->search, $this->searchableFields)
...
?>
<x-layouts.app :title="__('Master : Stages')">
@volt('apps.master.stages.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-stage" wire:click="$dispatch('stage-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Stages</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Stages</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Master\StageTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.master.stage"
        ...
        <livewire:apps.form.master.stage/>
    </flux:main>
@endvolt
</x-layouts.app>
