...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Skill Categories', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Skill Categories', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'skill-category-export'],
    ],
]);
...
$storeUserFilter = function (\Livewire\Component $component) use ($currentViewSettings): void {
    ...
    UserFilter::updateOrCreate(
        ['user_id' => $userId, 'key' => 'skill-categories'],
        ['value' => [
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    ...
    return Excel::download(new SkillCategoryExport($rows), 'skill-categories.xlsx');
};

$deleteSkillCategory = function (int $recordId): bool {
    $record = SkillCategory::query()->findOrFail($recordId);
    $record->delete();
    ...
    $this->dispatch(
        ...
        message: 'The skill category has been successfully deleted.'
    );
...
on([
    'skill-category-saved' => function () {
    },
    'skill-category-export' => $export,
    'skill-category-delete-confirmed' => $deleteSkillCategory,
]);

mount(function () use ($applyViewSettings, $currentViewSettings) {
    $filter = UserFilter::where('user_id', auth()->id())
        ->where('key', 'skill-categories')
        ->first();
...
$skillCategories = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
...
?>
<x-layouts.app :title="__('Master : Skill Categories')">
@volt('apps.master.skill-categories.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-skill-category" wire:click="$dispatch('skill-category-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Skill Categories</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Skill Categories</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Master\SkillCategoryTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                :columns="$columns"
                :rows="$this->skillCategories"
                :sort-field="$this->sortField"
                :sort-direction="$this->sortDirection"
                sort-action="sortBy"
                actions-view="pages.apps.actions.master.skill-category"
        ...
        <livewire:apps.form.master.skill-category/>
    </flux:main>
@endvolt
</x-layouts.app>
