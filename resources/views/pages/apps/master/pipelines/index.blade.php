...
use App\Exports\Master\PipelineExport;
use App\Models\Master\Pipeline;
use App\Models\User\UserFilter;
use App\Tables\Master\PipelineTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Pipelines', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Pipelines', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'pipeline-export'],
    ],
]);
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    ...
    return Excel::download(new PipelineExport($rows), 'pipelines.xlsx');
};

$deletePipeline = function (int $recordId): bool {
    $pipeline = Pipeline::query()->findOrFail($recordId);
    $pipeline->delete();
    ...
    $this->dispatch(
        ...
        message: 'The pipeline has been successfully deleted.'
    );
...
on([
    'pipeline-saved' => function () {
    },
    'pipeline-export' => $export,
    'pipeline-delete-confirmed' => $deletePipeline,
]);
...
$pipelines = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
...
?>
<x-layouts.app :title="__('Master : Pipelines')">
@volt('apps.master.pipelines.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-pipeline" wire:click="$dispatch('pipeline-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Pipelines</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Pipelines</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Master\PipelineTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.master.pipeline"
        ...
        <livewire:apps.form.master.pipeline/>
    </flux:main>
@endvolt
</x-layouts.app>
