...
use App\Exports\Master\SkillExport;
use App\Models\HR\Skill;
use App\Models\User\UserFilter;
use App\Tables\Master\SkillTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Skills', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Skills', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'skill-export'],
    ],
]);
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    ...
    return Excel::download(new SkillExport($rows), 'skills.xlsx');
};

$deleteSkill = function (int $recordId): bool {
    $record = Skill::query()->findOrFail($recordId);
    $record->delete();
...
on([
    ...
    'skill-export' => $export,
    'skill-delete-confirmed' => $deleteSkill,
]);
...
$skills = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
    return Skill::query()
        ->with('skillCategory')
        ->search($this->search, $this->searchableFields)
...
?>
<x-layouts.app :title="__('Master : Skills')">
@volt('apps.master.skills.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-skill" wire:click="$dispatch('skill-create')">
        ...
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Skills</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Master\SkillTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.master.skill"
        ...
        <livewire:apps.form.master.skill/>
    </flux:main>
@endvolt
</x-layouts.app>
