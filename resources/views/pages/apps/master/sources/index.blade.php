...
use App\Exports\Master\SourceExport;
use App\Models\CRM\Source;
use App\Models\User\UserFilter;
use App\Tables\Master\SourceTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Sources', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Sources', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'source-export'],
    ],
]);
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    ...
    return Excel::download(new SourceExport($rows), 'sources.xlsx');
};

$deleteSource = function (int $recordId): bool {
    $record = Source::query()->findOrFail($recordId);
    $record->delete();
...
on([
    'source-saved' => function () {
    },
    'source-export' => $export,
    'source-delete-confirmed' => $deleteSource,
]);
...
$sources = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
    return Source::query()
...
?>
<x-layouts.app :title="__('Master : Sources')">
@volt('apps.master.sources.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-source" wire:click="$dispatch('source-create')">
        ...
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Sources</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Master\SourceTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.master.source"
        ...
        <livewire:apps.form.master.source/>
    </flux:main>
@endvolt
</x-layouts.app>
