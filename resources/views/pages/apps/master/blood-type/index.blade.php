...
use App\Exports\Master\BloodTypeExport;
use App\Models\Master\BloodType;
use App\Models\User\UserFilter;
use App\Tables\Master\BloodTypeTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Blood Types', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Blood Types', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'blood-type-export'],
    ],
]);
...
$storeUserFilter = function (\Livewire\Component $component) use ($currentViewSettings): void {
    ...
    UserFilter::updateOrCreate(
        ['user_id' => $userId, 'key' => 'blood-types'],
        ['value' => [
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    $rows = BloodType::query()
        ->search($this->search, $this->searchableFields)
        ->get();

    return Excel::download(new BloodTypeExport($rows), 'blood-types.xlsx');
};

$deleteBloodType = function (int $recordId): bool {
    $bloodType = BloodType::query()->findOrFail($recordId);
    $bloodType->delete();

    $this->dispatch('modal-close', name: 'delete');
    $this->dispatch(
        'notify',
        title: 'Blood type deleted',
        message: 'The blood type has been successfully deleted.'
    );
...
on([
    'blood-type-saved' => function () {
    },
    'blood-type-export' => $export,
    'blood-type-delete-confirmed' => $deleteBloodType,
]);

mount(function () use ($applyViewSettings, $currentViewSettings) {
    $filter = UserFilter::where('user_id', auth()->id())
        ->where('key', 'blood-types')
        ->first();
...
$bloodTypes = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
...
?>
<x-layouts.app :title="__('Master : Blood Types')">
@volt('apps.master.blood-type.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-blood-type" wire:click="$dispatch('blood-type-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Blood Types</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Blood Types</flux:heading>
                <flux:input
        ...
        <livewire:apps.form.master.blood-type/>
    </flux:main>
@endvolt
</x-layouts.app>
