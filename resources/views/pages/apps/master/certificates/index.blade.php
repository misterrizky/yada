...
use App\Exports\Master\CertificateExport;
use App\Models\HR\Certificate;
use App\Models\User\UserFilter;
use App\Tables\Master\CertificateTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Certificates', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Certificates', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'certificate-export'],
    ],
]);
...
$storeUserFilter = function (\Livewire\Component $component) use ($currentViewSettings): void {
    ...
    UserFilter::updateOrCreate(
        ['user_id' => $userId, 'key' => 'certificates'],
        ['value' => [
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    ...
    return Excel::download(new CertificateExport($rows), 'certificates.xlsx');
};

$deleteCertificate = function (int $recordId): bool {
    $certificate = Certificate::query()->findOrFail($recordId);
    $certificate->delete();
...
on([
    'certificate-saved' => function () {
    },
    'certificate-export' => $export,
    'certificate-delete-confirmed' => $deleteCertificate,
]);
...
$certificates = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
...
?>
<x-layouts.app :title="__('Master : Certificates')">
@volt('apps.master.certificates.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-certificate" wire:click="$dispatch('certificate-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Certificates</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Certificates</flux:heading>
                <flux:input
        ...
        <livewire:apps.form.master.certificate/>
    </flux:main>
@endvolt
</x-layouts.app>
