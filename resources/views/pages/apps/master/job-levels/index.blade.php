...
use App\Exports\Master\JobLevelExport;
use App\Models\HR\JobLevel;
use App\Models\User\UserFilter;
use App\Tables\Master\JobLevelTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Job Levels', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Job Levels', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'job-level-export'],
    ],
]);
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    ...
    return Excel::download(new JobLevelExport($rows), 'job-levels.xlsx');
};

$deleteJobLevel = function (int $recordId): bool {
    $record = JobLevel::query()->findOrFail($recordId);
    $record->delete();

    $this->dispatch('modal-close', name: 'delete');
    $this->dispatch(
        'notify',
        title: 'Job Level deleted',
        message: 'The job level has been successfully deleted.'
    );
...
on([
    'job-level-saved' => function () {
    },
    'job-level-export' => $export,
    'job-level-delete-confirmed' => $deleteJobLevel,
]);

mount(function () use ($applyViewSettings, $currentViewSettings) {
    $filter = UserFilter::where('user_id', auth()->id())
        ->where('key', 'job-levels')
        ->first();
...
$jobLevels = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
...
?>
<x-layouts.app :title="__('Master : Job Levels')">
@volt('apps.master.job-levels.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-job-level" wire:click="$dispatch('job-level-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Job Levels</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Job Levels</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Master\JobLevelTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.master.job-level"
        ...
        <livewire:apps.form.master.job-level/>
    </flux:main>
@endvolt
</x-layouts.app>
