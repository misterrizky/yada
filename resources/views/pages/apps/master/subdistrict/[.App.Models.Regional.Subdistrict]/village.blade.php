...
use App\Exports\Regional\VillageExport;
use App\Models\Regional\Village;
use App\Models\User\UserFilter;
use App\Tables\Regional\VillageTable as TableConfig;
use Maatwebsite\Excel\Facades\Excel;
use function Laravel\Folio\name;
use function Livewire\Volt\{computed, mount, on, state, usesPagination};

usesPagination();
name('app.subdistrict.show-village');

state([
    'subdistrict' => fn () => $subdistrict,
    'search' => '',
    'sortField' => '',
    'sortDirection' => 'asc',
    'perPage' => 10,
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Villages', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Villages', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'village-export'],
    ],
]);
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    $query = $this->subdistrict
        ? $this->subdistrict->villages()->getQuery()
        : Village::query();
    ...
    return Excel::download(new VillageExport($rows), 'villages.xlsx');
};

$deleteVillage = function (int $recordId): bool {
    $village = Village::query()->findOrFail($recordId);
    $village->delete();
    ...
    $this->dispatch(
        ...
        message: 'The village has been successfully deleted.'
    );
...
on([
    'village-saved' => function () {
        // Refresh otomatis terjadi karena state berubah atau dipanggil ulang
    },
    'village-export' => $export,
    'village-delete-confirmed' => $deleteVillage,
]);

mount(function () use ($applyViewSettings, $currentViewSettings) {
    if ($this->subdistrict) {
        $this->subdistrict->loadMissing('city.state.country');
    }
...
$villages = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
    $query = $this->subdistrict
        ? $this->subdistrict->villages()->getQuery()
        : Village::query();
...
$chartData = computed(fn (): \Illuminate\Support\Collection => Village::query()
    ->when($this->subdistrict, fn ($q) => $q->where('subdistrict_id', $this->subdistrict->id))
    ->selectRaw('subdistrict_id, COUNT(*) as total')
    ->groupBy('subdistrict_id')
    ->orderBy('subdistrict_id')
    ->get()
);
?>
<x-layouts.app :title="__('Regional : Village')">
@volt
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-village" wire:click="$dispatch('village-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>{{ $this->subdistrict->city?->state?->country?->name ?? 'Country' }}</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>State</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>{{ $this->subdistrict->city?->state?->name ?? 'State' }}</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>City</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>{{ $this->subdistrict->city?->name ?? 'City' }}</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>Subdistrict</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>{{ $this->subdistrict->name }}</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>Village</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Village</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Regional\VillageTable::columns()"
                :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.regional.village"
        ...
        <livewire:apps.form.regional.village/>
    </flux:main>
@endvolt
</x-layouts.app>
