...
use App\Exports\Master\DegreeExport;
use App\Models\Master\Degree;
use App\Models\User\UserFilter;
use App\Tables\Master\DegreeTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Degrees', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Degrees', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'degree-export'],
    ],
]);
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    ...
    return Excel::download(new DegreeExport($rows), 'degrees.xlsx');
};

$deleteDegree = function (int $recordId): bool {
    $degree = Degree::query()->findOrFail($recordId);
    $degree->delete();
    ...
    $this->dispatch(
        ...
        message: 'The degree has been successfully deleted.'
    );
...
on([
    'degree-saved' => function () {
    },
    'degree-export' => $export,
    'degree-delete-confirmed' => $deleteDegree,
]);
...
$degrees = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
    return Degree::query()
        ->search($this->search, $this->searchableFields)
...
?>
<x-layouts.app :title="__('Master : Degrees')">
@volt('apps.master.degree.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-degree" wire:click="$dispatch('degree-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Degrees</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Degrees</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Master\DegreeTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.master.degree"
        ...
        <livewire:apps.form.master.degree/>
    </flux:main>
@endvolt
</x-layouts.app>
