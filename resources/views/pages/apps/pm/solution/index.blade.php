...
use App\Exports\Project\SolutionExport;
use App\Models\Master\Solution;
use App\Models\User\UserFilter;
use App\Tables\Project\SolutionTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Solutions', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Solutions', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'solution-export'],
    ],
]);
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    ...
    return Excel::download(new SolutionExport($rows), 'solutions.xlsx');
};

$deleteSolution = function (int $recordId): bool {
    $record = Solution::query()->findOrFail($recordId);
    $record->delete();
    ...
    $this->dispatch(
        ...
        message: 'The solution has been successfully deleted.'
    );
...
on([
    'solution-saved' => function () {
    },
    'solution-export' => $export,
    'solution-delete-confirmed' => $deleteSolution,
]);
...
$solutions = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
...
?>
<x-layouts.app :title="__('Project : Solutions')">
@volt('apps.pm.solution.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-solution" wire:click="$dispatch('solution-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Solutions</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Solutions</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Project\SolutionTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.pm.solution"
        ...
        <livewire:apps.form.pm.solution/>
    </flux:main>
@endvolt
</x-layouts.app>
