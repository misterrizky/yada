...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Solution Categories', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Solution Categories', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'solution-category-export'],
    ],
]);
...
$storeUserFilter = function (\Livewire\Component $component) use ($currentViewSettings): void {
    ...
    UserFilter::updateOrCreate(
        ['user_id' => $userId, 'key' => 'solution-categories'],
        ['value' => [
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    $rows = SolutionCategory::query()
        ->with('parent')
        ->search($this->search, $this->searchableFields)
        ->get();

    return Excel::download(new SolutionCategoryExport($rows), 'solution-categories.xlsx');
};

$deleteSolutionCategory = function (int $recordId): bool {
    $record = SolutionCategory::query()->findOrFail($recordId);
    $record->delete();

    $this->dispatch('modal-close', name: 'delete');
    $this->dispatch(
        'notify',
        title: 'Solution Category deleted',
        message: 'The solution category has been successfully deleted.'
    );
...
on([
    'solution-category-saved' => function () {
    },
    'solution-category-export' => $export,
    'solution-category-delete-confirmed' => $deleteSolutionCategory,
]);

mount(function () use ($applyViewSettings, $currentViewSettings) {
    $filter = UserFilter::where('user_id', auth()->id())
        ->where('key', 'solution-categories')
        ->first();
...
$solutionCategories = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
...
?>
<x-layouts.app :title="__('Project : Solution Categories')">
@volt('apps.pm.solution-category.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-solution-category" wire:click="$dispatch('solution-category-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item href="#">Project</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>Solution Categories</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Solution Categories</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Project\SolutionCategoryTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                :columns="$columns"
                :rows="$this->solutionCategories"
                :sort-field="$this->sortField"
                :sort-direction="$this->sortDirection"
                sort-action="sortBy"
                actions-view="pages.apps.actions.pm.solution-category"
        ...
        <livewire:apps.form.pm.solution-category/>
    </flux:main>
@endvolt
</x-layouts.app>
