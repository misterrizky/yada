...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Solution Units', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Solution Units', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'solution-unit-export'],
    ],
]);
...
$storeUserFilter = function (\Livewire\Component $component) use ($currentViewSettings): void {
    ...
    UserFilter::updateOrCreate(
        ['user_id' => $userId, 'key' => 'solution-units'],
        ['value' => [
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    $rows = SolutionUnit::query()

        ->search($this->search, $this->searchableFields)
        ->get();

    return Excel::download(new SolutionUnitExport($rows), 'solution-units.xlsx');
};

$deleteSolutionUnit = function (int $recordId): bool {
    $record = SolutionUnit::query()->findOrFail($recordId);
    $record->delete();

    $this->dispatch('modal-close', name: 'delete');
    $this->dispatch(
        'notify',
        title: 'Solution Unit deleted',
        message: 'The solution unit has been successfully deleted.'
    );
...
on([
    'solution-unit-saved' => function () {
    },
    'solution-unit-export' => $export,
    'solution-unit-delete-confirmed' => $deleteSolutionUnit,
]);

mount(function () use ($applyViewSettings, $currentViewSettings) {
    $filter = UserFilter::where('user_id', auth()->id())
        ->where('key', 'solution-units')
        ->first();
...
$solutionUnits = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
    return SolutionUnit::query()
...
?>
<x-layouts.app :title="__('Project : Solution Units')">
@volt('apps.pm.solution-unit.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-solution-unit" wire:click="$dispatch('solution-unit-create')">
                    <flux:button size="sm" variant="primary" color="sky" icon="plus">
                        Create Solution Unit
                    </flux:button>
                </flux:modal.trigger>
            </div>
        @endpush
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Solution Units</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Solution Units</flux:heading>
                <flux:input
        ...
        <livewire:apps.form.pm.solution-unit/>
    </flux:main>
@endvolt
</x-layouts.app>
