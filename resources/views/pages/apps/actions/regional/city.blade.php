@php
    $deletePayload = [
        ...
        'confirmEvent' => 'city-delete-confirmed',
        'recordId' => $row->id,
        'confirmLabel' => 'Delete',
    ];
@endphp
...
<flux:dropdown>
    ...
    <flux:menu>
        <flux:menu.item :href="route('app.city.show-subdistrict', ['city' => $row->id])" wire:navigate icon="eye">
            View Subdistrict
        </flux:menu.item>
        <flux:menu.item
            icon="pencil-square"
            wire:click="$dispatch('city-edit', { cityId: {{ $row->id }} })">
            Edit
        </flux:menu.item>
        <flux:menu.separator />
        <flux:menu.item
