@php
    $deletePayload = [
        'title' => 'Delete Order',
        'message' => 'You are about to delete ' . $row->order_number . '. This action cannot be reversed.',
        'confirmEvent' => 'order-delete-confirmed',
        'recordId' => $row->id,
        'confirmLabel' => 'Delete',
    ];
@endphp
...
<flux:dropdown>
    ...
    <flux:menu>
        <flux:menu.item
            icon="pencil-square"
            wire:click="$dispatch('order-edit', { orderId: {{ $row->id }} })">
            Edit
        </flux:menu.item>
        <flux:menu.separator />
        <flux:menu.item
