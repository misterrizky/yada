...
use App\Exports\Achievement\LevelExport;
use App\Models\User\UserFilter;
use App\Tables\Achievement\LevelTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Levels', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Levels', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'level-export'],
    ],
]);
...
$export = function () use ($applySearch): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    $query = Level::query()->withCount('users');

    $applySearch($query, $this->search, $this->searchableFields);

    return Excel::download(new LevelExport($query->get()), 'levels.xlsx');
};

$deleteLevel = function (int $recordId): bool {
    $level = Level::query()->findOrFail($recordId);
    $level->delete();
...
on([
    'level-saved' => function () {
    },
    'level-export' => $export,
    'level-delete-confirmed' => $deleteLevel,
]);
...
$levels = computed(function () use ($applySearch): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
    $query = Level::query()->withCount('users');
...
?>
<x-layouts.app :title="__('Achievements : Levels')">
@volt('apps.achievements.level.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-level" wire:click="$dispatch('level-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Levels</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Levels</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Achievement\LevelTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.achievements.level"
        ...
        <livewire:apps.form.achievement.level/>
    </flux:main>
@endvolt
</x-layouts.app>
