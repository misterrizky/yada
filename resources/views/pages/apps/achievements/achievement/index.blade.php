...
use App\Exports\Achievement\AchievementExport;
use App\Models\User\UserFilter;
use App\Tables\Achievement\AchievementTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Achievements', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Achievements', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'achievement-export'],
    ],
]);
...
$storeUserFilter = function (\Livewire\Component $component) use ($currentViewSettings): void {
    ...
    UserFilter::updateOrCreate(
        ['user_id' => $userId, 'key' => 'achievements'],
        ['value' => [
...
$export = function () use ($applySearch): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    $query = Achievement::query()->withCount('users');

    $applySearch($query, $this->search, $this->searchableFields);

    return Excel::download(new AchievementExport($query->get()), 'achievements.xlsx');
};

$deleteAchievement = function (int $recordId): bool {
    $achievement = Achievement::query()->findOrFail($recordId);
    $achievement->delete();
...
on([
    'achievement-saved' => function () {
    },
    'achievement-export' => $export,
    'achievement-delete-confirmed' => $deleteAchievement,
]);
...
$achievements = computed(function () use ($applySearch): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
    $query = Achievement::query()->withCount('users');
...
?>
<x-layouts.app :title="__('Achievements : Achievements')">
@volt('apps.achievements.achievement.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-achievement" wire:click="$dispatch('achievement-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Achievements</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Achievements</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Achievement\AchievementTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.achievements.achievement"
        ...
        <livewire:apps.form.achievement.achievement/>
    </flux:main>
@endvolt
</x-layouts.app>
