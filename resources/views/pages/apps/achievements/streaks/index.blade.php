...
use App\Exports\Achievement\StreakExport;
use App\Models\User\UserFilter;
use App\Tables\Achievement\StreakTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Streaks', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Streaks', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'streak-export'],
    ],
]);
...
$applySearch = function (\Illuminate\Database\Eloquent\Builder $query, ?string $keyword, array $fields): void {
    if (! filled($keyword) || $fields === []) {
        return;
    }
...
$applyHistoryCount = function (\Illuminate\Database\Eloquent\Builder $query): void {
    $streakTable = (new Streak())->getTable();

    $query->addSelect([
        'histories_count' => StreakHistory::query()
            ->selectRaw('count(*)')
            ->whereColumn('streak_histories.user_id', $streakTable . '.user_id')
            ->whereColumn('streak_histories.activity_id', $streakTable . '.activity_id'),
    ]);
};

$export = function () use ($applySearch, $applyHistoryCount): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    $query = Streak::query()->with(['user', 'activity']);

    $applyHistoryCount($query);
    $applySearch($query, $this->search, $this->searchableFields);

    return Excel::download(new StreakExport($query->get()), 'streaks.xlsx');
};

$deleteStreak = function (int $recordId): bool {
    $streak = Streak::query()->findOrFail($recordId);
    $streak->delete();
    ...
    $this->dispatch(
        ...
        message: 'The streak has been successfully deleted.'
    );
...
on([
    'streak-saved' => function () {
    },
    'streak-export' => $export,
    'streak-delete-confirmed' => $deleteStreak,
]);
...
$streaks = computed(function () use ($applySearch, $applyHistoryCount): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
    $query = Streak::query()->with(['user', 'activity']);

    $applyHistoryCount($query);
    $applySearch($query, $this->search, $this->searchableFields);

    $query->when($this->sortField, fn ($q) =>
...
?>
<x-layouts.app :title="__('Achievements : Streaks')">
@volt('apps.achievements.streaks.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-streak" wire:click="$dispatch('streak-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item href="#">Achievements</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>Streaks</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Streaks</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Achievement\StreakTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.achievements.streak"
        ...
        <livewire:apps.form.achievement.streak/>
    </flux:main>
@endvolt
</x-layouts.app>
