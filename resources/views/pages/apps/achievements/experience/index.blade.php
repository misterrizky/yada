...
use App\Exports\Achievement\ExperienceExport;
use App\Models\User\UserFilter;
use App\Tables\Achievement\ExperienceTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Experiences', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Experiences', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'experience-export'],
    ],
]);
...
$storeUserFilter = function (\Livewire\Component $component) use ($currentViewSettings): void {
    ...
    UserFilter::updateOrCreate(
        ['user_id' => $userId, 'key' => 'experiences'],
        ['value' => [
...
$applyAuditCount = function (\Illuminate\Database\Eloquent\Builder $query): void {
    $experienceTable = (new Experience())->getTable();

    $query->addSelect([
        'audits_count' => ExperienceAudit::query()
            ->selectRaw('count(*)')
            ->whereColumn('experience_audits.user_id', $experienceTable . '.user_id'),
    ]);
};

$export = function () use ($applySearch, $applyAuditCount): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    $query = Experience::query()->with(['user', 'status']);

    $applyAuditCount($query);
    $applySearch($query, $this->search, $this->searchableFields);

    return Excel::download(new ExperienceExport($query->get()), 'experiences.xlsx');
};

$deleteExperience = function (int $recordId): bool {
    $experience = Experience::query()->findOrFail($recordId);
    $experience->delete();
...
$experiences = computed(function () use ($applySearch, $applyAuditCount): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
    $query = Experience::query()->with(['user', 'status']);
...
?>
<x-layouts.app :title="__('Achievements : Experience')">
@volt('apps.achievements.experience.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-experience" wire:click="$dispatch('experience-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Experience</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Experience</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Achievement\ExperienceTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                :columns="$columns"
                :rows="$this->experiences"
                :sort-field="$this->sortField"
                :sort-direction="$this->sortDirection"
                sort-action="sortBy"
                actions-view="pages.apps.actions.achievements.experience"
        ...
        <livewire:apps.form.achievement.experience/>
    </flux:main>
@endvolt
</x-layouts.app>
