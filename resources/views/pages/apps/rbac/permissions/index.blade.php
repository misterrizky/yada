...
use App\Exports\Rbac\PermissionExport;
use App\Models\User\UserFilter;
use App\Tables\Rbac\PermissionTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Permissions', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Permissions', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'permission-export'],
    ],
]);
...
$storeUserFilter = function (\Livewire\Component $component) use ($currentViewSettings): void {
    ...
    UserFilter::updateOrCreate(
        ['user_id' => $userId, 'key' => 'permissions'],
        ['value' => [
...
$export = function () use ($applySearch): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    ...
    return Excel::download(new PermissionExport($query->get()), 'permissions.xlsx');
};

$deletePermission = function (int $recordId): bool {
    $permission = Permission::query()->findOrFail($recordId);
    $permission->delete();
    ...
    $this->dispatch(
        ...
        message: 'The permission has been successfully deleted.'
    );
...
on([
    ...
    'permission-delete-confirmed' => $deletePermission,
]);
...
$permissions = computed(function () use ($applySearch): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
    $query = Permission::query();
...
?>
<x-layouts.app :title="__('RBAC : Permissions')">
@volt('apps.rbac.permissions.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-permission" wire:click="$dispatch('permission-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item href="#">RBAC</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>Permissions</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Permissions</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Rbac\PermissionTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.rbac.permission"
        ...
        <livewire:apps.form.rbac.permission/>
    </flux:main>
@endvolt
</x-layouts.app>
