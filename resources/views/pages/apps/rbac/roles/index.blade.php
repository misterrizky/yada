...
use App\Exports\Rbac\RoleExport;
use App\Models\User\UserFilter;
use App\Tables\Rbac\RoleTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Roles', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Roles', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'role-export'],
    ],
]);
...
$export = function () use ($applySearch): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    ...
    return Excel::download(new RoleExport($query->get()), 'roles.xlsx');
};

$deleteRole = function (int $recordId): bool {
...
on([
    ...
    'role-export' => $export,
    'role-delete-confirmed' => $deleteRole,
]);
...
$roles = computed(function () use ($applySearch): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
...
?>
<x-layouts.app :title="__('RBAC : Roles')">
@volt('apps.rbac.roles.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-role" wire:click="$dispatch('role-create')">
        ...
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Roles</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Rbac\RoleTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.rbac.role"
