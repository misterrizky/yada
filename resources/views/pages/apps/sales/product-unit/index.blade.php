...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Product Units', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Product Units', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'product-unit-export'],
    ],
]);
...
$storeUserFilter = function (\Livewire\Component $component) use ($currentViewSettings): void {
    ...
    UserFilter::updateOrCreate(
        ['user_id' => $userId, 'key' => 'product-units'],
        ['value' => [
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    ...
    return Excel::download(new ProductUnitExport($rows), 'product-units.xlsx');
};

$deleteProductUnit = function (int $recordId): bool {
    $record = ProductUnit::query()->findOrFail($recordId);
    $record->delete();

    $this->dispatch('modal-close', name: 'delete');
    $this->dispatch(
        'notify',
        title: 'Product Unit deleted',
        message: 'The product unit has been successfully deleted.'
    );
...
on([
    'product-unit-saved' => function () {
    },
    'product-unit-export' => $export,
    'product-unit-delete-confirmed' => $deleteProductUnit,
]);

mount(function () use ($applyViewSettings, $currentViewSettings) {
    $filter = UserFilter::where('user_id', auth()->id())
        ->where('key', 'product-units')
        ->first();
...
$productUnits = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
...
?>
<x-layouts.app :title="__('Sales : Product Units')">
@volt('apps.sales.product-unit.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-product-unit" wire:click="$dispatch('product-unit-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Product Units</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Product Units</flux:heading>
                <flux:input
        ...
        <livewire:apps.form.sales.product-unit/>
    </flux:main>
@endvolt
</x-layouts.app>
