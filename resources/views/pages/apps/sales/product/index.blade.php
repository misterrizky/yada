...
use App\Exports\Sales\ProductExport;
use App\Models\Master\Product;
use App\Models\User\UserFilter;
use App\Tables\Sales\ProductTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Products', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Products', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'product-export'],
    ],
]);
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    ...
    return Excel::download(new ProductExport($rows), 'products.xlsx');
};

$deleteProduct = function (int $recordId): bool {
    $record = Product::query()->findOrFail($recordId);
    $record->delete();
...
on([
    ...
    'product-delete-confirmed' => $deleteProduct,
]);
...
$products = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
...
?>
<x-layouts.app :title="__('Sales : Products')">
@volt('apps.sales.product.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-product" wire:click="$dispatch('product-create')">
        ...
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Products</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Sales\ProductTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.sales.product"
        ...
        <livewire:apps.form.sales.product/>
    </flux:main>
@endvolt
</x-layouts.app>
