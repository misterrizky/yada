...
use App\Exports\Sales\ProductCategoryExport;
use App\Models\Master\ProductCategory;
use App\Models\User\UserFilter;
use App\Tables\Sales\ProductCategoryTable as TableConfig;
use Maatwebsite\Excel\Facades\Excel;
use function Laravel\Folio\name;
use function Livewire\Volt\{computed, mount, on, state, usesPagination};

usesPagination();
name('app.product-category');
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Product Categories', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Product Categories', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'product-category-export'],
    ],
]);
...
$storeUserFilter = function (\Livewire\Component $component) use ($currentViewSettings): void {
    ...
    UserFilter::updateOrCreate(
        ['user_id' => $userId, 'key' => 'product-categories'],
        ['value' => [
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    ...
    return Excel::download(new ProductCategoryExport($rows), 'product-categories.xlsx');
};

$deleteProductCategory = function (int $recordId): bool {
    $record = ProductCategory::query()->findOrFail($recordId);
    $record->delete();
    ...
    $this->dispatch(
        ...
        message: 'The product category has been successfully deleted.'
    );
...
on([
    'product-category-saved' => function () {
    },
    'product-category-export' => $export,
    'product-category-delete-confirmed' => $deleteProductCategory,
]);

mount(function () use ($applyViewSettings, $currentViewSettings) {
    $filter = UserFilter::where('user_id', auth()->id())
        ->where('key', 'product-categories')
        ->first();
...
$productCategories = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
...
?>
<x-layouts.app :title="__('Sales : Product Categories')">
@volt('apps.sales.product-category.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-product-category" wire:click="$dispatch('product-category-create')">
        ...
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Product Categories</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Sales\ProductCategoryTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                :columns="$columns"
                :rows="$this->productCategories"
                :sort-field="$this->sortField"
                :sort-direction="$this->sortDirection"
                sort-action="sortBy"
                actions-view="pages.apps.actions.sales.product-category"
        ...
        <livewire:apps.form.sales.product-category/>
    </flux:main>
@endvolt
</x-layouts.app>
