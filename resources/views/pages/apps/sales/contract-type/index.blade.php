...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Contract Types', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Contract Types', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'contract-type-export'],
    ],
]);
...
$storeUserFilter = function (\Livewire\Component $component) use ($currentViewSettings): void {
    ...
    UserFilter::updateOrCreate(
        ['user_id' => $userId, 'key' => 'contract-types'],
        ['value' => [
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    $rows = ContractType::query()

        ->search($this->search, $this->searchableFields)
        ->get();

    return Excel::download(new ContractTypeExport($rows), 'contract-types.xlsx');
};

$deleteContractType = function (int $recordId): bool {
    $record = ContractType::query()->findOrFail($recordId);
    $record->delete();
    ...
    $this->dispatch(
        ...
        message: 'The contract type has been successfully deleted.'
    );
...
on([
    'contract-type-saved' => function () {
    },
    'contract-type-export' => $export,
    'contract-type-delete-confirmed' => $deleteContractType,
]);

mount(function () use ($applyViewSettings, $currentViewSettings) {
    $filter = UserFilter::where('user_id', auth()->id())
        ->where('key', 'contract-types')
        ->first();
...
$contractTypes = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
...
?>
<x-layouts.app :title="__('Sales : Contract Types')">
@volt('apps.sales.contract-type.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-contract-type" wire:click="$dispatch('contract-type-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Contract Types</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Contract Types</flux:heading>
                <flux:input
        ...
        <livewire:apps.form.sales.contract-type/>
    </flux:main>
@endvolt
</x-layouts.app>
