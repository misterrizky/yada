...
use App\Exports\Sales\QuotationItemExport;
use App\Models\Sales\QuotationItem;
use App\Models\User\UserFilter;
use App\Tables\Sales\QuotationItemTable as TableConfig;
use Maatwebsite\Excel\Facades\Excel;
use function Laravel\Folio\name;
use function Livewire\Volt\{computed, mount, on, state, usesPagination};

usesPagination();
name('app.quotation.show-item');

state([
    'quotation' => fn () => $quotation,
    'search' => '',
    'sortField' => '',
    'sortDirection' => 'asc',
    'perPage' => 10,
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Quotation Items', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Quotation Items', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'quotation-item-export'],
    ],
]);
...
$storeUserFilter = function (\Livewire\Component $component) use ($currentViewSettings): void {
    ...
    UserFilter::updateOrCreate(
        ['user_id' => $userId, 'key' => 'quotation-items'],
        ['value' => [
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    $rows = QuotationItem::query()
        ->with(['product', 'solution', 'quotation'])
        ->search($this->search, $this->searchableFields)
        ->when($this->quotation, fn ($q) => $q->where('quotation_id', $this->quotation->id))
        ->get();

    return Excel::download(new QuotationItemExport($rows), 'quotation-items.xlsx');
};

$deleteQuotationItem = function (int $recordId): bool {
    $record = QuotationItem::query()->findOrFail($recordId);
    $record->delete();
    ...
    $this->dispatch(
        ...
        message: 'The quotation item has been successfully deleted.'
    );
...
on([
    'quotation-item-saved' => function () {
    },
    'quotation-item-export' => $export,
    'quotation-item-delete-confirmed' => $deleteQuotationItem,
]);

mount(function () use ($applyViewSettings, $currentViewSettings) {
    $filter = UserFilter::where('user_id', auth()->id())
        ->where('key', 'quotation-items')
        ->first();
...
$quotationItems = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
    $query = $this->quotation
        ? $this->quotation->quotationItems()->getQuery()
        : QuotationItem::query();
...
?>
<x-layouts.app :title="__('Sales : Quotation Items')">
@volt('apps.sales.quotation.items.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-quotation-item" wire:click="$dispatch('quotation-item-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item href="#">Sales</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>Quotation</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>{{ $this->quotation->quotation_number }}</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>Quotation Items</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Quotation Items</flux:heading>
                <flux:input
        ...
        <livewire:apps.form.sales.quotation-item/>
    </flux:main>
@endvolt
</x-layouts.app>
