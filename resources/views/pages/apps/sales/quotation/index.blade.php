...
use App\Exports\Sales\QuotationExport;
use App\Models\Sales\Quotation;
use App\Models\User\UserFilter;
use App\Tables\Sales\QuotationTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Quotations', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Quotations', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'quotation-export'],
    ],
]);
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    $rows = Quotation::query()
        ->with(['lead', 'company', 'proposal', 'currency'])
        ->search($this->search, $this->searchableFields)
        ->get();

    return Excel::download(new QuotationExport($rows), 'quotations.xlsx');
};

$deleteQuotation = function (int $recordId): bool {
    $record = Quotation::query()->findOrFail($recordId);
    $record->delete();
    ...
    $this->dispatch(
        ...
        message: 'The quotation has been successfully deleted.'
    );
...
on([
    'quotation-saved' => function () {
    },
    'quotation-export' => $export,
    'quotation-delete-confirmed' => $deleteQuotation,
]);

mount(function () use ($applyViewSettings, $currentViewSettings) {
    $filter = UserFilter::where('user_id', auth()->id())
        ->where('key', 'quotations')
        ->first();
...
$quotations = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
...
?>
<x-layouts.app :title="__('Sales : Quotations')">
@volt('apps.sales.quotation.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-quotation" wire:click="$dispatch('quotation-create')">
        ...
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Quotations</flux:heading>
                <flux:input
        ...
        <livewire:apps.form.sales.quotation/>
    </flux:main>
@endvolt
</x-layouts.app>
