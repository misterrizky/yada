...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Proposal Items', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Proposal Items', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'proposal-item-export'],
    ],
]);
...
$storeUserFilter = function (\Livewire\Component $component) use ($currentViewSettings): void {
    ...
    UserFilter::updateOrCreate(
        ['user_id' => $userId, 'key' => 'proposal-items'],
        ['value' => [
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    $rows = ProposalItem::query()
        ->with(['product', 'solution', 'proposal'])
        ->search($this->search, $this->searchableFields)
        ->when($this->proposal, fn ($q) => $q->where('proposal_id', $this->proposal->id))
        ->get();

    return Excel::download(new ProposalItemExport($rows), 'proposal-items.xlsx');
};

$deleteProposalItem = function (int $recordId): bool {
    $record = ProposalItem::query()->findOrFail($recordId);
    $record->delete();

    $this->dispatch('modal-close', name: 'delete');
    $this->dispatch(
        'notify',
        title: 'Proposal Item deleted',
        message: 'The proposal item has been successfully deleted.'
    );
...
on([
    'proposal-item-saved' => function () {
    },
    'proposal-item-export' => $export,
    'proposal-item-delete-confirmed' => $deleteProposalItem,
]);

mount(function () use ($applyViewSettings, $currentViewSettings) {
    $filter = UserFilter::where('user_id', auth()->id())
        ->where('key', 'proposal-items')
        ->first();
...
$proposalItems = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
    $query = $this->proposal
        ? $this->proposal->proposalItems()->getQuery()
        : ProposalItem::query();
...
?>
<x-layouts.app :title="__('Sales : Proposal Items')">
@volt('apps.sales.proposal.items.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-proposal-item" wire:click="$dispatch('proposal-item-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Proposal</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>{{ $this->proposal->proposal_number }}</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>Proposal Items</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Proposal Items</flux:heading>
                <flux:input
        ...
        <livewire:apps.form.sales.proposal-item/>
    </flux:main>
@endvolt
</x-layouts.app>
