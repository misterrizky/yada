...
use App\Exports\Sales\ProposalExport;
use App\Models\Sales\Proposal;
use App\Models\User\UserFilter;
use App\Tables\Sales\ProposalTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Proposals', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Proposals', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'proposal-export'],
    ],
]);
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    $rows = Proposal::query()
        ->with(['lead', 'company', 'currency'])
        ->search($this->search, $this->searchableFields)
        ->get();

    return Excel::download(new ProposalExport($rows), 'proposals.xlsx');
};

$deleteProposal = function (int $recordId): bool {
    $record = Proposal::query()->findOrFail($recordId);
    $record->delete();
...
on([
    'proposal-saved' => function () {
    },
    'proposal-export' => $export,
    'proposal-delete-confirmed' => $deleteProposal,
]);
...
$proposals = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
...
?>
<x-layouts.app :title="__('Sales : Proposals')">
@volt('apps.sales.proposal.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-proposal" wire:click="$dispatch('proposal-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Proposals</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Proposals</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Sales\ProposalTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.sales.proposal"
        ...
        <livewire:apps.form.sales.proposal/>
    </flux:main>
@endvolt
</x-layouts.app>
