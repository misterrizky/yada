...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Order Items', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Order Items', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'order-item-export'],
    ],
]);
...
$storeUserFilter = function (\Livewire\Component $component) use ($currentViewSettings): void {
    ...
    UserFilter::updateOrCreate(
        ['user_id' => $userId, 'key' => 'order-items'],
        ['value' => [
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    $rows = OrderItem::query()
        ->with(['product', 'solution', 'order'])
        ->search($this->search, $this->searchableFields)
        ->when($this->order, fn ($q) => $q->where('order_id', $this->order->id))
        ->get();

    return Excel::download(new OrderItemExport($rows), 'order-items.xlsx');
};

$deleteOrderItem = function (int $recordId): bool {
    $record = OrderItem::query()->findOrFail($recordId);
    $record->delete();
...
on([
    'order-item-saved' => function () {
    },
    'order-item-export' => $export,
    'order-item-delete-confirmed' => $deleteOrderItem,
]);

mount(function () use ($applyViewSettings, $currentViewSettings) {
    $filter = UserFilter::where('user_id', auth()->id())
        ->where('key', 'order-items')
        ->first();
...
$orderItems = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
    $query = $this->order
...
?>
<x-layouts.app :title="__('Sales : Order Items')">
@volt('apps.sales.order.items.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-order-item" wire:click="$dispatch('order-item-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Order</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>{{ $this->order->order_number }}</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>Order Items</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Order Items</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Sales\OrderItemTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.sales.order-item"
        ...
        <livewire:apps.form.sales.order-item/>
    </flux:main>
@endvolt
</x-layouts.app>
