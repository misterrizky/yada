...
use App\Tables\Sales\OrderTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Orders', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Orders', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'order-export'],
    ],
]);
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    $rows = Order::query()
        ->with(['company', 'quotation', 'contract', 'currency'])
        ->search($this->search, $this->searchableFields)
        ->get();

    return Excel::download(new OrderExport($rows), 'orders.xlsx');
};

$deleteOrder = function (int $recordId): bool {
    $record = Order::query()->findOrFail($recordId);
    $record->delete();
...
on([
    ...
    'order-export' => $export,
    'order-delete-confirmed' => $deleteOrder,
]);
...
$orders = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
    return Order::query()
        ->with(['company', 'quotation', 'contract', 'currency'])
        ->search($this->search, $this->searchableFields)
...
?>
<x-layouts.app :title="__('Sales : Orders')">
@volt('apps.sales.order.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-order" wire:click="$dispatch('order-create')">
        ...
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Orders</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Sales\OrderTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.sales.order"
        ...
        <livewire:apps.form.sales.order/>
    </flux:main>
@endvolt
</x-layouts.app>
