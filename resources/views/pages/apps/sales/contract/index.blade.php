...
use App\Exports\Sales\ContractExport;
use App\Models\Sales\Contract;
use App\Models\User\UserFilter;
use App\Tables\Sales\ContractTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Contracts', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Contracts', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'contract-export'],
    ],
]);
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    $rows = Contract::query()
        ->with(['company', 'project', 'quotation', 'contractType', 'currency'])
        ->search($this->search, $this->searchableFields)
        ->get();

    return Excel::download(new ContractExport($rows), 'contracts.xlsx');
};

$deleteContract = function (int $recordId): bool {
    $record = Contract::query()->findOrFail($recordId);
    $record->delete();
...
on([
    ...
    'contract-delete-confirmed' => $deleteContract,
]);
...
$contracts = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
...
?>
<x-layouts.app :title="__('Sales : Contracts')">
@volt('apps.sales.contract.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-contract" wire:click="$dispatch('contract-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Contracts</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Contracts</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\Sales\ContractTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.sales.contract"
        ...
        <livewire:apps.form.sales.contract/>
    </flux:main>
@endvolt
</x-layouts.app>
