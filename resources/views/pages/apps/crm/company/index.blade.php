...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Companies', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Companies', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'company-export'],
    ],
]);
...
$storeUserFilter = function (\Livewire\Component $component) use ($currentViewSettings): void {
    ...
    UserFilter::updateOrCreate(
        ['user_id' => $userId, 'key' => 'crm-companies'],
        ['value' => [
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    $rows = Company::query()
        ->with(['industry', 'currency', 'source', 'accountManager'])
        ->search($this->search, $this->searchableFields)
        ->get();

    return Excel::download(new CompanyExport($rows), 'companies.xlsx');
};

$deleteCompany = function (int $recordId): bool {
    $record = Company::query()->findOrFail($recordId);
    $record->delete();
...
$companies = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
...
?>
<x-layouts.app :title="__('CRM : Companies')">
@volt('apps.crm.companies.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-company" wire:click="$dispatch('company-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Companies</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Companies</flux:heading>
                <flux:input
        ...
        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\CRM\CompanyTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.crm.company"
        ...
        <livewire:apps.form.crm.company/>
    </flux:main>
@endvolt
</x-layouts.app>
