...
use App\Exports\CRM\LeadExport;
use App\Kanbans\CRM\LeadKanban;
use App\Models\CRM\Lead;
use App\Models\User\UserFilter;
use App\Tables\CRM\LeadTable as TableConfig;
...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Leads', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Leads', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'lead-export'],
    ],
]);
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    $rows = Lead::query()
        ->with(['source', 'stage', 'industry', 'user', 'company'])
        ->search($this->search, $this->searchableFields)
        ->get();

    return Excel::download(new LeadExport($rows), 'leads.xlsx');
};

$deleteLead = function (int $recordId): bool {
    $record = Lead::query()->findOrFail($recordId);
    $record->delete();
...
on([
    ...
    'lead-export' => $export,
    'lead-delete-confirmed' => $deleteLead,
]);
...
$leads = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
    return Lead::query()
        ->with(['source', 'stage', 'industry', 'user', 'company'])
        ->search($this->search, $this->searchableFields)
...
$kanbanColumns = computed(function (): array {
    return (new LeadKanban())->columns($this->search);
});
?>
<x-layouts.app :title="__('CRM : Leads')">
@volt('apps.crm.leads.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-lead" wire:click="$dispatch('lead-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Leads</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Leads</flux:heading>
                <flux:input
        ...
        <div class="pb-8">
            <x-app.kanban
                ...
                card-view="pages.apps.crm.leads._kanban-card"
                empty-text="No leads in this stage yet."
            />
        </div>

        <div wire:loading wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-app.skeleton.table
                :columns="\App\Tables\CRM\LeadTable::columns()" :rows="10"
            />
        </div>
        <div wire:loading.remove wire:target="search,sortBy,perPage,saveViewSettings,gotoPage,nextPage,previousPage">
            <x-data-table
                ...
                sort-action="sortBy"
                actions-view="pages.apps.actions.crm.lead"
