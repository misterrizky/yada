...
state([
    ...
    'headerDropdownItems' => fn () => [
        ['label' => 'Import Lost Reasons', 'icon' => 'arrow-up-tray', 'clickable' => false],
        ['label' => 'Export Lost Reasons', 'icon' => 'arrow-down-tray', 'clickable' => true, 'click' => 'lost-reason-export'],
    ],
]);
...
$storeUserFilter = function (\Livewire\Component $component) use ($currentViewSettings): void {
    ...
    UserFilter::updateOrCreate(
        ['user_id' => $userId, 'key' => 'crm-lost-reasons'],
        ['value' => [
...
$export = function (): \Symfony\Component\HttpFoundation\BinaryFileResponse {
    $rows = LostReason::query()
        ->search($this->search, $this->searchableFields)
        ->get();

    return Excel::download(new LostReasonExport($rows), 'lost-reasons.xlsx');
};

$deleteLostReason = function (int $recordId): bool {
    $record = LostReason::query()->findOrFail($recordId);
    $record->delete();

    $this->dispatch('modal-close', name: 'delete');
    $this->dispatch(
        'notify',
        title: 'Lost reason deleted',
        message: 'The lost reason has been successfully deleted.'
    );
...
on([
    'lost-reason-saved' => function () {
    },
    'lost-reason-export' => $export,
    'lost-reason-delete-confirmed' => $deleteLostReason,
]);

mount(function () use ($applyViewSettings, $currentViewSettings) {
    $filter = UserFilter::where('user_id', auth()->id())
        ->where('key', 'crm-lost-reasons')
        ->first();
...
$lostReasons = computed(function (): \Illuminate\Contracts\Pagination\LengthAwarePaginator {
...
?>
<x-layouts.app :title="__('CRM : Lost Reasons')">
@volt('apps.crm.lost-reason.index')
    @php
    ...
    <flux:main>
        @push('app-header-actions')
            <div class="mx-5 flex items-center gap-2">
                ...
                <flux:modal.trigger name="form-lost-reason" wire:click="$dispatch('lost-reason-create')">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item href="#">CRM</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>Lost Reasons</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                <flux:heading class="mt-5" size="xl" level="1">Lost Reasons</flux:heading>
                <flux:input
        ...
        <livewire:apps.form.crm.lost-reason/>
    </flux:main>
@endvolt
</x-layouts.app>
