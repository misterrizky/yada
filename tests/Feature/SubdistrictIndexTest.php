...
class SubdistrictIndexTest extends TestCase
{
    use RefreshDatabase;

    public function test_subdistrict_index_shows_subdistricts_for_selected_city(): void
    {
        ...
        $this->makeSubdistrict($city, '3174', '3174.01', 'Kebayoran Baru');
        $this->makeSubdistrict($otherCity, '0603', '0603.10', 'Hollywood');

        $response = $this->actingAs($user)
            ->get(route('app.city.show-subdistrict', ['city' => $city->id]));

        $response->assertOk();
        $response->assertSee('Kebayoran Baru');
        $response->assertDontSee('Hollywood');
    }
