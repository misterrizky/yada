...
class SolutionUnitSeeder extends Seeder
{
    public function run(): void
    {
        // 1. Seed Units
        $unitData = json_decode(file_get_contents(base_path('database/seeders/data/master/solution_units.json')), true);
        foreach ($unitData as $unitItem) {
            $unit = SolutionUnit::firstOrCreate(
                [
                    'ulid' => Str::ulid(),
                    'name' => $unitItem['name'],
